...
export default function HomePageContent() {
  ...
  useEffect(() => {
    ...
    api.listCategories().then((cats) => {
      const allCategories = ["Story / Cartoon", ...cats.filter(c => c !== "Story / Cartoon")];
  ...
  async function createProject() {
    ...
    try {
      ...
      if (category === "Story / Cartoon") {
        finalFormat = "image_story" as VideoFormat;
      }
      const project = await api.createProject(idea, category, finalFormat, videoLength, language, subtitlesEnabled, subtitleLanguage);
              ...
              <div className="mt-8 flex flex-wrap items-end gap-6">
                  ...
                  <div className="relative">
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        if (e.target.value === "Story / Cartoon") {
                          setFormat("image_story");
                        }
                      }}
                    >
                      {categories.map((cat) => (
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex rounded-xl bg-white/[0.03] p-1.5 border border-white/5 backdrop-blur-md">
                    {[
                      { id: "shorts", label: "Shorts", icon: "📱", title: "" },
                      { id: "long", label: "Long Form", icon: "🎬", title: "" },
                    ].map((v) => (
                      <button
                        key={v.id}
                        style={{ paddingLeft: '1.5rem', paddingRight: '1.5rem', paddingTop: '0.625rem', paddingBottom: '0.625rem' }}
                        className={`flex items-center gap-2 rounded-lg text-sm font-bold transition-all duration-300 ${format === v.id ? "bg-forge-red text-white shadow-lg shadow-forge-red/30 scale-105" : "text-zinc-500 hover:text-zinc-300 hover:bg-white/5"}`}
                          ...
                          <div className="flex items-start justify-between">
                            <div className="space-y-3">
                              ...
                              <div className="flex flex-wrap gap-2">
                                ...
                                <Badge tone="red">{project.video_format === "shorts" ? "Shorts" : project.video_format === "image_story" ? "Story / Cartoon" : "Long Video"}</Badge>
                              ...
                              <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-wider text-zinc-600" suppressHydrationWarning>
                                {new Date(project.created_at).toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' })} • {new Date(project.created_at).toLocaleTimeString(undefined, { hour: '2-digit', minute: '2-digit' })}
                              </div>
                            </div>
